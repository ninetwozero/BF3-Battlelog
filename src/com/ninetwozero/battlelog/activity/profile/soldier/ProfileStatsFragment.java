...
public class ProfileStatsFragment extends Bf3Fragment implements
		DefaultFragment, OnCloseListDialogListener {
	...
	private TextView personaName, rankTitle, rankId, currentLevelPoints,
			nextLevelPoints, pointsToMake;
	...
	private void setSelectedPersonaVariables() {
		...
		callURI = UriFactory.personaOverview(mSelectedPersona,
				mSelectedPlatformId);
	}

	private long getSelectedPersonaId(int position) {
		String idsString = mSharedPreferences.getString(
				Constants.SP_BL_PERSONA_ID, "");
	...
	private String getSelectedPersonaName(int position) {
		String names = mSharedPreferences.getString(
				Constants.SP_BL_PERSONA_NAME, "");
	...
	private boolean hasRankData() {
		Cursor cursor = getContext().getContentResolver().query(
				RankProgress.URI, RankProgress.RANK_PROGRESS_PROJECTION,
				RankProgress.Columns.PERSONA_ID + "=?",
				new String[] { String.valueOf(mSelectedPersona) }, null);
	...
	public void findViews() {
		...
		if (personaArrayLength() == 1) {
			view.findViewById(R.id.img_persona_list).setVisibility(
					View.INVISIBLE);
		}
		...
		// Progress
		currentLevelPoints = (TextView) view
				.findViewById(R.id.string_progress_curr);
		nextLevelPoints = (TextView) view
				.findViewById(R.id.string_progress_max);
		pointsToMake = (TextView) view.findViewById(R.id.string_progress_left);

		personaStatisticsTable = (TableLayout) view
				.findViewById(R.id.persona_statistics);
		scoreStatisticsTable = (TableLayout) view
				.findViewById(R.id.score_statistics);
	...
	private void populateRankProgress() {
		...
		personaName.setText(rankProgress.getPersonaName() + " "
				+ rankProgress.getPlatform());
		rankTitle.setText(fromResource(rankProgress.getRank()));
		rankId.setText(String.valueOf(rankProgress.getRank()));

		// Progress
		mProgressBar
				.setMax((int) (rankProgress.getNextRankScore() - rankProgress
						.getCurrentRankScore()));
		mProgressBar.setProgress((int) (rankProgress.getScore() - rankProgress
				.getCurrentRankScore()));
		currentLevelPoints.setText(String.valueOf(rankProgress.getScore()
				- rankProgress.getCurrentRankScore()));
		nextLevelPoints.setText(String.valueOf(rankProgress.getNextRankScore()
				- rankProgress.getCurrentRankScore()));
		pointsToMake.setText(String.valueOf(rankProgress.getNextRankScore()
				- rankProgress.getScore()));
	}

	private void populateStatistics(List<Statistics> statistics,
			TableLayout layout) {
	...
	private void updateRankProgressDB(PersonaInfo pi) {
		...
		getContext().getContentResolver().insert(RankProgress.URI,
				rankProgressForDB(pi, mSelectedPersona));
	}

	private void updatePersonaStats(PersonaInfo pi) {
		listPersonaStatistics = personaStatisticsFromJSON(pi);
		getContext().getContentResolver().insert(PersonaStatistics.URI,
				personaStatisticsForDB(pi, mSelectedPersona));
	}

	private void updateScoreStatistics(PersonaInfo pi) {
		listScoreStatistics = scoreStatisticsFromJSON(pi);
		getContext().getContentResolver().insert(ScoreStatistics.URI,
				scoreStatisticsForDB(pi, mSelectedPersona));
	}
	...
	public boolean handleSelectedOption(MenuItem item) {
		...
		if (item.getItemId() == R.id.option_compare) {
			...
			new Intent(mContext, UnlockActivity.class).putExtra("profile",
					mProfileData).putExtra("selectedPosition", position));
		}
		return true;
	}
	...
	public void reload() {
		/*
		 * TODO can't use this method implementation because for some reason it
		 * been called before actual content showed. Very weird. I would
		 * recommend to delete DB record of currently selected persona and then
		 * call getData() method
		 */
		// getLoaderManager().restartLoader(0, bundle, this);
	}

	private void startLoadingDialog() { // TODO extract multiple duplicates of
										// same code
