...
public class DateTimePreference extends ListPreference {
    ...
    private CharSequence[] entries() {
        return isDateFormat() ? listOfDates() : ProfileSettings.TIME_FORMATS;
    }

    private CharSequence[] entryValues() {
        return isDateFormat() ? ProfileSettings.DATE_FORMAT_VALUES : ProfileSettings.TIME_FORMAT_VALUES;
    }

    private CharSequence[] listOfDates() {
        List<String> list = new ArrayList<String>();
        Date today = new Date(Calendar.getInstance().getTimeInMillis());
        for (String pattern : ProfileSettings.DATE_PATTERNS) {
